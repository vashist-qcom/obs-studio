	...
	obs_data_t *settings)
{
	...
	HEVCRateControl rateControl = (HEVCRateControl)obs_data_get_int(
		settings, MFP_RATE_CONTROL);

	if (rateControl == HEVCRateControlCBR ||
	    rateControl == HEVCRateControlVBR) {
...
static void *MFHEVC_Create(obs_data_t *settings, obs_encoder_t *encoder)
{
	...
	enc->hevcEncoder.reset(new HEVCEncoder(encoder,
			enc->descriptor,
			enc->width,
			enc->height,
			enc->framerateNum,
...
static bool MFHEVC_Encode(void *data, struct encoder_frame *frame,
		struct encoder_packet *packet, bool *received_packet)
{
	...
	if (!enc->profiler_encode)
		 enc->profiler_encode = profile_store_name(
				...
				"MFHEVC_Encode(%s)", enc->descriptor->Name());

	ProfileScope(enc->profiler_encode);

	*received_packet = false;

	if (!enc->hevcEncoder->ProcessInput(frame->data, frame->linesize,
			frame->pts, &status))
		return false;
	...
	if (!enc->hevcEncoder->ProcessOutput(&outputData, &outputDataLength,
