...
#define MFTEXT(x) obs_module_text("MF.AV1." x)
...
static const char *MFAV1_GetName(void *type_data)
{
	...
	obs_data_t *settings)
{
	...
	AV1RateControl rateControl = (AV1RateControl)obs_data_get_int(
		settings, MFP_RATE_CONTROL);

	if (rateControl == AV1RateControlCBR ||
	    rateControl == AV1RateControlVBR) {
...
static obs_properties_t *MFAV1_GetProperties(void *)
{
...
static void MFAV1_GetDefaults(obs_data_t *settings)
{
#define PROP_DEF(x, y, z) obs_data_set_default_ ## x(settings, y, z)
	...
	PROP_DEF(int,    MFP_RATE_CONTROL,    AV1RateControlCBR);
	PROP_DEF(int,    MFP_PROFILE,         AV1ProfileMain);
...
static void UpdateParams(MFAV1_Encoder *enc, obs_data_t *settings)
{
...
#define PROP_GET(x, y, z) (z)obs_data_get_ ## x(settings, y)
	enc->profile       = PROP_GET(int,  MFP_PROFILE,      AV1Profile);
	enc->rateControl   = PROP_GET(int,  MFP_RATE_CONTROL, AV1RateControl);
...
static bool ApplyCBR(MFAV1_Encoder *enc)
{
...
static bool ApplyVBR(MFAV1_Encoder *enc)
{
	enc->av1Encoder->SetBitrate(enc->bitrate);
...
static void *MFAV1_Create(obs_data_t *settings, obs_encoder_t *encoder)
{
	...
	enc->av1Encoder.reset(new AV1Encoder(encoder,
			enc->descriptor,
			enc->width,
			enc->height,
			enc->framerateNum,
...
static void MFAV1_Destroy(void *data)
{
	MFAV1_Encoder *enc = static_cast<MFAV1_Encoder *>(data);
	delete enc;
}

static bool MFAV1_Encode(void *data, struct encoder_frame *frame,
		struct encoder_packet *packet, bool *received_packet)
{
	...
	if (!enc->profiler_encode)
		 enc->profiler_encode = profile_store_name(
				...
				"MFAV1_Encode(%s)", enc->descriptor->Name());

	ProfileScope(enc->profiler_encode);

	*received_packet = false;

	if (!enc->av1Encoder->ProcessInput(frame->data, frame->linesize,
			frame->pts, &status))
		return false;
	...
	if (!enc->av1Encoder->ProcessOutput(&outputData, &outputDataLength,
...
static bool MFAV1_GetExtraData(void *data, uint8_t **extra_data, size_t *size)
{
	MFAV1_Encoder *enc = static_cast<MFAV1_Encoder *>(data);
...
static bool MFAV1_GetSEIData(void *data, uint8_t **sei_data, size_t *size)
{
...
static void MFAV1_GetVideoInfo(void *, struct video_scale_info *info)
{
	info->format = VIDEO_FORMAT_NV12;
}

static bool MFAV1_Update(void *data, obs_data_t *settings)
{
	MFAV1_Encoder *enc = static_cast<MFAV1_Encoder *>(data);
