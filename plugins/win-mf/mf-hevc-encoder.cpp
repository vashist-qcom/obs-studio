...
HEVCEncoder::HEVCEncoder(const obs_encoder_t *encoder,
...
HEVCEncoder::~HEVCEncoder()
{
	...
	while (inputRequests == 0) {
		...
		if (hr != MF_E_TRANSFORM_NEED_MORE_INPUT && FAILED(hr)) {
			MF_LOG_COM(LOG_ERROR, "HEVCEncoder::~HEVCEncoder: "
					"ProcessOutput()", hr);
			break;
		}
...
HRESULT HEVCEncoder::CreateMediaTypes(ComPtr<IMFMediaType> &i,
...
//bool HEVCEncoder::SetEntropyEncoding(HEVCEntropyEncoding entropyEncoding)
//{
//	HRESULT hr;
//
//	if (codecApi) {
//		HR_CHECK(LOG_WARNING, SetCodecProperty(codecApi,
//				CODECAPI_AVEncHEVCCABACEnable,
//				entropyEncoding == HEVCEntropyEncodingCABAC));
//	}
//
//	return true;
//
...
HRESULT HEVCEncoder::CreateEmptySample(ComPtr<IMFSample> &sample,
...
bool HEVCEncoder::ProcessInput(UINT8 **data, UINT32 *linesize, UINT64 pts,
		Status *status)
{
...
HRESULT HEVCEncoder::ProcessOutput()
{
	...
	frame.reset(new HEVCFrame(keyframe,
			samplePts / sampleDur,
			sampleDts / sampleDur,
			std::move(data)));
...
bool HEVCEncoder::ProcessOutput(UINT8 **data, UINT32 *dataLength,
